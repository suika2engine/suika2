 ...
 *  - We need the following file for a build:
 *    - src/m2/iemote.h:
 *      - Comes with a proper copy of the E-mote SDK
 *  - We need the following files for a run:
 *    - emotedriver.dll:
 *      - Comes with a proper copy of the E-mote SDK (You need an NDA to obtain this file)
 *    - d3dx9_43.dll:
 *      - Comes with the DirectX End-User Runtime June 2010
 *
 * [Disclamer]
 *  - E-mote is a trademark of M2 Co., Ltd.
 *  - E-mote is a proprietary intellectual property of M2 Co., Ltd.
 *  - Keiichi Tabata wrote this source code under an NDA contract with M2 Co., Ltd.
 *  - Keiichi Tabata claims no intellectual property on this E-mote support
 *  - Suika2 users who utilize the E-mote support **MUST** acquire a valid license from M2 Co., Ltd.
 */
...
bool init_motion(void)
{
    ...
    // Load EmoteCreate().
    //  - We don't link emotedriver.dll to suika.exe or suika-pro.exe
    //  - Instead, we load emotedriver.dll dynamically
	//  - Thus, emotedriver.dll is the sole plugin DLL for our E-mote support
