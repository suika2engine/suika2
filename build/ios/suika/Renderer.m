...
*/
    struct image *img = create_image_from_file("ch", "001-fun.png");
    MTLTextureDescriptor *textureDescriptor = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA8Unorm width:get_image_width(img) height:get_image_height(img) mipmapped:NO];
    id <MTLTexture> texture = [_device newTextureWithDescriptor:textureDescriptor];
    MTLRegion region = MTLRegionMake2D(0, 0, get_image_width(img), get_image_height(img));
    [texture replaceRegion:region mipmapLevel:0 withBytes:get_image_pixels(img) bytesPerRow:get_image_width(img)*4];
    _colorMap = texture;
    if(!_colorMap || error)
    {
    ...
    struct wave *w = create_wave_from_file("bgm", "01.ogg", true);
...
//    matrix_float4x4 modelMatrix = matrix4x4_rotation(_rotation, rotationAxis);
    matrix_float4x4 modelMatrix = matrix4x4_translation(0, 0, 0);
