...
まずはじめに、Suika 2のバイナリを [ダウンロード](http://luxion.jp/s2/dl.html) します。ここではWindows版を前提とします。ダウンロードしたZIPファイルを展開すると、下記のようなフォルダとファイルが作成されます。これは、Suika 2の実行ファイルとサンプルゲームが一緒になったものです。
...
* init.txt</td><td>second.txt</td></tr>
```
@bg roof.png 1.0
@ch center 001-fun.png 1.0
init.txtの中にいます。
