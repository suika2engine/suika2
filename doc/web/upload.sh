...
{
    if [ ! -z "$GTAG" ]; then
        sed -e 's|.*INSERT-GTAG-HERE.*|\<script async src="https://www.googletagmanager.com/gtag/js\?id=G-PLYR5Y3JSJ"></script><script>window.dataLayer = window.dataLayer || [];function gtag(){dataLayer.push(arguments);}gtag("js", new Date());gtag("config", "G-PLYR5Y3JSJ");\</script\>|g' $1 > tmp
        ftp-upload.sh tmp $1;
    else
        ftp-upload.sh $1 $1;
    fi
